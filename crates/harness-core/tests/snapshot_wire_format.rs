...
fn snapshot_agent_event_session_started() {
    let event = AgentEvent::SessionStarted {
        session_id: "s1".into(), profile_id: "p1".into(),
    };
    let json = serde_json::to_string(&event).unwrap();
    assert_eq!(json, r#"{"type":"session_started","session_id":"s1","profile_id":"p1"}"#);
}

#[test]
fn snapshot_agent_event_result() {
    let event = AgentEvent::Result { content: "done".into(), is_error: false };
    let json = serde_json::to_string(&event).unwrap();
    assert_eq!(json, r#"{"type":"result","content":"done","is_error":false}"#);
}

#[test]
fn snapshot_agent_event_session_ended() {
    let event = AgentEvent::SessionEnded {
        session_id: "s1".into(), synthetic: true,
        termination_reason: TerminationReason::Completed,
        result_received: true, process_exit_received: true,
    };
    let json = serde_json::to_string(&event).unwrap();
...
fn snapshot_agent_event_raw_vendor() {
    ...
    let json = serde_json::to_string(&event).unwrap();
    assert_eq!(json, r#"{"type":"raw_vendor_event","raw_type":"vendor.future","payload":{"x":1}}"#);
}
...
fn snapshot_task_lifecycle_terminal_count() {
    let terminals = [
        TaskLifecycle::Pending, TaskLifecycle::Ready, TaskLifecycle::Dispatched,
        TaskLifecycle::Running, TaskLifecycle::AwaitingInput, TaskLifecycle::RetryPending,
        TaskLifecycle::Submitted, TaskLifecycle::Verified,
        TaskLifecycle::Done, TaskLifecycle::Cancelled,
        TaskLifecycle::Superseded, TaskLifecycle::Failed,
    ].iter().filter(|s| s.is_terminal()).count();
    assert_eq!(terminals, 4, "Task: 4 terminal states (Done, Cancelled, Superseded, Failed)");
}

#[test]
fn snapshot_execution_lifecycle_all_terminal() {
    for t in &[ExecutionLifecycle::Completed, ExecutionLifecycle::Failed, ExecutionLifecycle::Lost, ExecutionLifecycle::Cancelled] {
...
fn snapshot_taskresult_wire() {
    let result = harness_core::contracts::task_result::TaskResult {
        ...
        checks: vec![harness_core::contracts::task_result::TaskResultCheck {
            command: "cargo test".into(), exit_code: 0, output_ref: None,
        }],
        blockers: vec![], risks: vec![], proposed_followups: vec![],
    };
...
fn snapshot_taskenvelope_wire() {
    let envelope = harness_core::contracts::task_envelope::TaskEnvelope {
        ...
        budget: harness_core::contracts::task_envelope::TaskBudget {
            max_turns: 10, max_time_ms: 30000, max_cost_cents: None,
        },
