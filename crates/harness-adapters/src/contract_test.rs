...
fn test_profile() -> RuntimeProfile {
    RuntimeProfile {
        ...
        agent_definition_id: "ct-def".into(),
        label: "Contract Test".into(),
...
impl TestSink {
    ...
    pub fn into_inner(self) -> Vec<AgentEvent> {
...
impl AdapterContractTest {
    pub async fn run(adapter: &dyn AgentAdapter) -> Vec<ContractTestResult> {
        vec![
            Self::test_detect(adapter).await,
            Self::test_get_version(adapter).await,
            Self::test_inspect_config(adapter).await,
            Self::test_auth(adapter).await,
            Self::test_probe(adapter).await,
            Self::test_session_lifecycle(adapter).await,
    ...
    async fn test_probe(adapter: &dyn AgentAdapter) -> ContractTestResult {
        ...
        match adapter.probe(&tmp).await {
            ...
            Ok(r) => ContractTestResult::fail(
                "probe",
                format!("smoke_test_passed: {}", r.smoke_test_passed),
            ),
    ...
    async fn test_session_lifecycle(adapter: &dyn AgentAdapter) -> ContractTestResult {
            ...
            .await
        {
            Ok(s) => s,
            Err(e) => {
                return ContractTestResult::fail("session_lifecycle", format!("start_session: {e}"))
            }
        };
        ...
        if !session.is_active() {
            return ContractTestResult::fail("session_lifecycle", "not active".into());
        }
        ...
        match result {
            Ok(()) if events.is_empty() => {
                ContractTestResult::fail("session_lifecycle", "no events".into())
            }
            Ok(()) => ContractTestResult::pass("session_lifecycle"),
            Err(e) if events.is_empty() => {
                ContractTestResult::fail("session_lifecycle", format!("failed no events: {e}"))
            }
    ...
    async fn test_dispose_idempotent(adapter: &dyn AgentAdapter) -> ContractTestResult {
        ...
        {
            Ok(s) => s,
            Err(e) => return ContractTestResult::fail("dispose", format!("start: {e}")),
        };
