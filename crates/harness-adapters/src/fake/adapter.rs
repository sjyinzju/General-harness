...
impl AgentSession for FakeAgentSession {
    ...
    async fn receive_events(
        &mut self,
        sink: &mut dyn harness_core::contracts::agent_adapter::AgentEventSink,
    ) -> Result<(), CoreError> {
        let script = self
            .script
            .as_ref()
        ...
        for file_op in &script.files_to_create {
            ...
            std::fs::write(&file_op.path, &file_op.content).map_err(|e| {
                CoreError::new(
                    ErrorCode::Internal,
                    format!("FakeAgent file write: {e}"),
                    ErrorSource::System,
                )
            })?;
        }
