...
async fn test_fc01_loop_insert_before_effect() {
    ...
    assert!(lc.load(Ordering::SeqCst) <= 1,
        "FC01: before-effect fault must prevent double-create: count={}", lc.load(Ordering::SeqCst));
    let _ = result;
}
...
async fn test_fc02_loop_insert_response_lost() {
    let (db, gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::LoopInsert, FaultKind::ResponseLostAfterSuccess);
    let lc = Arc::new(AtomicUsize::new(0));
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp)
    ...
    assert!(lc.load(Ordering::SeqCst) <= 1,
        "FC02: response-lost must not double-create: count={}", lc.load(Ordering::SeqCst));
}
...
async fn test_fc03_ownership_before_effect() {
    let (db, gw) = setup().await;
    let CreateLoopOutcome::Created { loop_id } =
        TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw.clone())
        .create_loop(&loop_req("ik-fc03", "hfc03")).await.unwrap()
...
async fn test_fc04_ownership_response_lost() {
    ...
    assert!(
        ...
        "FC04: response-lost ownership must be idempotent: {:?}", r
    );
}
...
async fn test_fc05_stale_takeover_response_lost() {
    ...
    let CreateLoopOutcome::Created { loop_id } =
        svc.create_loop(&loop_req("ik-fc05", "hfc05")).await.unwrap()
    else { panic!("not created") };
    let r = svc.start_or_resume_loop(&loop_id, "owner1", 300).await;
...
async fn test_fc06_attempt_insert_before_effect() {
    ...
    let (loop_id, v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc06i", "hfc06i").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
        None,
    ).await;
...
async fn test_fc07_attempt_insert_response_lost() {
    ...
    let ac = Arc::new(AtomicUsize::new(0));
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::AttemptInsert, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp)
        .with_attempt_create_count(ac.clone());

    let (loop_id, _, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc07i", "hfc07i").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let _ = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
        None,
    ).await;
    assert!(ac.load(Ordering::SeqCst) <= 1,
        "FC07: response-lost must not double-create: count={}", ac.load(Ordering::SeqCst));
}
...
async fn test_fc12_execution_create_before_effect() {
    ...
    let r = svc.dispatch_attempt("ta-nonexistent", "t1", "prof-1", None, None, "ik-fc12", "hfc12").await;
    assert!(ec.load(Ordering::SeqCst) == 0 || r.is_err(),
...
async fn test_fc13_execution_create_response_lost() {
    let (db, gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::ExecutionCreate, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone())
        .with_fault_plan(fp);
    ...
    if let Ok(PrepareAttemptOutcome::Prepared { attempt_id, .. }) = r {
        let e1 = svc.dispatch_attempt(&attempt_id, "t1", "prof-1", None, None, "ik-e1", "he1").await;
        let e2 = svc.dispatch_attempt(&attempt_id, "t1", "prof-1", None, None, "ik-e2", "he2").await;
        if let (Ok(ex1), Ok(ex2)) = (&e1, &e2) {
            assert_eq!(ex1.execution_id, ex2.execution_id,
...
async fn test_fc20_decision_insert_before_effect() {
    ...
    let r = repo.insert_decision(
        "dec-fc20", "loop-fc20", "att-fc20",
        DecisionClassification::ContinueRepair,
        "[]", "", "", "", "", "",
        Some("prof-1"), None, "ik-fc20d", "hfc20d",
    ).await;
    assert!(r.is_err() || dc.load(Ordering::SeqCst) == 0,
...
async fn test_fc21_decision_response_lost() {
    ...
    let dc = Arc::new(AtomicUsize::new(0));
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::DecisionInsert, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_fault_plan(fp)
        .with_decision_count(dc.clone());

    let repo = TaskLoopRepo::new(db.pool.clone());
    let _ = repo.insert_decision(
        "dec-fc21a", "loop-fc21", "att-fc21",
        DecisionClassification::ContinueRepair,
        "[]", "", "", "", "", "",
        Some("prof-1"), None, "ik-fc21d", "hfc21d",
    ).await;
    let _ = repo.insert_decision(
        "dec-fc21b", "loop-fc21", "att-fc21",
        DecisionClassification::ContinueRepair,
        "[]", "", "", "", "", "",
        Some("prof-1"), None, "ik-fc21d", "hfc21d",
    ).await;
...
async fn test_fc22_context_pack_before_effect() {
    ...
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::ContextPackInsert, FaultKind::FailBeforeEffect);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_fault_plan(fp)
        .with_context_pack_count(cc.clone());

    let repo = TaskLoopRepo::new(db.pool.clone());
    let r = repo.insert_context_pack(
        "cp-fc22", "loop-fc22", None, 1, "{}", "{}", "fp22", None, "valid",
    ).await;
    assert!(r.is_err() || cc.load(Ordering::SeqCst) == 0,
...
async fn test_fc23_context_pack_response_lost() {
    ...
    let cc = Arc::new(AtomicUsize::new(0));
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::ContextPackInsert, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_fault_plan(fp)
        .with_context_pack_count(cc.clone());

    let repo = TaskLoopRepo::new(db.pool.clone());
    let _ = repo.insert_context_pack(
        "cp-fc23", "loop-fc23", None, 1, "{}", "{}", "fp23", None, "valid",
    ).await;
    assert!(cc.load(Ordering::SeqCst) <= 1,
        "FC23: response-lost must not double-insert CP: count={}", cc.load(Ordering::SeqCst));
}
...
async fn test_fc24_usage_write_before_effect() {
    ...
    let repo = TaskLoopRepo::new(db.pool.clone());
    let r = repo.insert_usage(
        "usage-fc24", "loop-fc24", "att-fc24", Some("exec-fc24"), "prof-1",
        Some("gpt-4"), Some("openai"),
        Some(100), Some(200), None, Some(5), Some(3000), Some(15000),
        "observed", true, Some("ufp24"), "ik-fc24u",
    ).await;
    assert!(r.is_err() || r == Ok(false),
        "FC24: before-effect must prevent usage write: {:?}", r);
}
...
async fn test_fc25_usage_response_lost() {
    let (db, _gw) = setup().await;
    let repo = TaskLoopRepo::new(db.pool.clone());
    let _ = repo.insert_usage(
        "usage-fc25a", "loop-fc25", "att-fc25", Some("exec-fc25"), "prof-1",
        Some("gpt-4"), Some("openai"),
        Some(100), Some(200), None, Some(5), Some(3000), Some(15000),
        "observed", true, Some("ufp25a"), "ik-fc25u",
    ).await;
    let r2 = repo.insert_usage(
        "usage-fc25b", "loop-fc25", "att-fc25", Some("exec-fc25"), "prof-1",
        Some("gpt-4"), Some("openai"),
        Some(100), Some(200), None, Some(5), Some(3000), Some(15000),
        "observed", true, Some("ufp25"), "ik-fc25u",
    ).await;
    ...
    assert!(usage.total_input_tokens.unwrap_or(0) <= 200,
        "FC25: response-lost usage must not double-count: {:?}", usage.total_input_tokens);
    let _ = r2;
}
...
async fn test_fc28_terminal_transition_response_lost() {
    let (db, gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::TerminalTransition, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp);

    let (loop_id, v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc28", "hfc28").await;
    ...
    assert!(
        ...
        "FC28: response-lost cancel must be idempotent: r1={:?} r2={:?}", r1, r2
    );
}
...
async fn test_fc29_terminal_event_response_lost() {
    let (db, _gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::EventWrite, FaultKind::ResponseLostAfterSuccess);
    ...
    assert!(ok, "FC29: response-lost event must be safe: r1={:?} r2={:?}", r1, r2);
}
...
async fn test_fc30_owner_fencing_change_before_effect() {
    ...
    let CreateLoopOutcome::Created { loop_id } =
        svc.create_loop(&loop_req("ik-fc30", "hfc30")).await.unwrap()
    else { panic!("not created") };
    let r = svc.start_or_resume_loop(&loop_id, "owner1", 300).await;
    // Owner takeover fault may or may not trigger depending on hook placement.
    match r {
...
async fn test_fc08_budget_reservation_before_effect() {
    ...
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::BudgetReservation, FaultKind::FailBeforeEffect);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp)
        .with_budget_reserve_count(brc.clone());

    let (loop_id, _v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc08", "hfc08").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let _ = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
    ...
    assert!(brc.load(Ordering::SeqCst) <= 1,
        "FC08: before-effect budget fault: count={}", brc.load(Ordering::SeqCst));
}
...
async fn test_fc09_budget_reservation_response_lost() {
    ...
    let brc = Arc::new(AtomicUsize::new(0));
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::BudgetReservation, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp)
        .with_budget_reserve_count(brc.clone());

    let (loop_id, _, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc09", "hfc09").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let _ = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
    ...
    assert!(brc.load(Ordering::SeqCst) <= 1,
        "FC09: response-lost budget: count={}", brc.load(Ordering::SeqCst));
}
...
async fn test_fc10_profile_selection_before_effect() {
    ...
    let (loop_id, _, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc10", "hfc10").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-outside-allowlist",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
        None,
    ).await;
    ...
    assert!(
        ...
        "FC10: disallowed profile must be rejected: {:?}", r
    );
}
...
async fn test_fc11_profile_selection_response_lost() {
    let (db, gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::ProfileSelection, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp);

    let (loop_id, _, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc11", "hfc11").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
    ...
    assert!(
        ...
        "FC11: response-lost profile must be retryable: {:?}", r
    );
}
...
async fn test_fc14_execution_binding_before_effect() {
    let (db, _gw) = setup().await;
    let repo = TaskLoopRepo::new(db.pool.clone());
    let r = repo.bind_execution("nonexistent-attempt", 1, "exec-x", AttemptLifecycle::Dispatched).await;
    assert!(r.is_err() || r == Ok(false),
        "FC14: binding nonexistent attempt must fail: {:?}", r);
}
...
async fn test_fc15_execution_binding_response_lost() {
    let (db, gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::ExecutionBind, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp);

    let (loop_id, _v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc15", "hfc15").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
    ...
    if let Ok(PrepareAttemptOutcome::Prepared { attempt_id, .. }) = r {
        ...
        match r2 {
            Ok(_) => {},
            Err(e) => assert!(e.contains("FOREIGN KEY") || e.contains("not found"),
                "FC15: response-lost bind error acceptable: {}", e),
        }
    }
}
...
async fn test_fc16_dispatch_before_effect() {
    ...
    let r = svc.dispatch_attempt("ta-nonexistent", "t1", "prof-1", None, None, "ik-fc16", "hfc16").await;
    assert!(r.is_err() || ec.load(Ordering::SeqCst) == 0,
        "FC16: before-effect dispatch must be blocked: {:?}", r);
}
...
async fn test_fc17_dispatch_response_lost() {
    ...
    let (loop_id, v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc17", "hfc17").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
    ...
    if let Ok(PrepareAttemptOutcome::Prepared { attempt_id, .. }) = r {
        let _ = svc.dispatch_attempt(&attempt_id, "t1", "prof-1", None, None, "ik-fc17d", "hfc17d").await;
        assert!(ec.load(Ordering::SeqCst) <= 1,
            "FC17: response-lost dispatch: count={}", ec.load(Ordering::SeqCst));
    }
}
...
async fn test_fc18_outcome_observation_failure() {
    ...
    match obs {
        Ok(o) => assert!(o.lifecycle.is_none(), "nonexistent exec must have no lifecycle"),
...
async fn test_fc19_dossier_read_failure() {
    ...
    let eligibility = harness_runtime::task_loop::validate_completion_eligibility(&db.pool, "nonexistent").await.unwrap();
...
async fn test_fc26_workspace_continuation_before_effect() {
    ...
    assert!(result.is_err(),
        "FC26: workspace continuation before effect must reject nonexistent path: {:?}", result);
}
...
async fn test_fc27_workspace_transfer_response_lost() {
    let (db, gw) = setup().await;
    let fp = Arc::new(FaultPlan::new());
    fp.inject_once(FaultBoundary::WorkspaceContinuation, FaultKind::ResponseLostAfterSuccess);
    let svc = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw)
        .with_fault_plan(fp);

    let (loop_id, _v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-fc27", "hfc27").await;
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, ft, "prof-1",
...
async fn test_two_pool_loop_creation_one_winner() {
    ...
    let lc = Arc::new(AtomicUsize::new(0));
    let s1 = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone()).with_loop_create_count(lc.clone());
    let s2 = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone()).with_loop_create_count(lc.clone());
...
async fn test_two_pool_attempt_creation_one_winner() {
    ...
    let ec = Arc::new(AtomicUsize::new(0));
    let s1 = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone()).with_attempt_create_count(ac.clone()).with_execution_count(ec.clone());
    let s2 = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone()).with_attempt_create_count(ac.clone()).with_execution_count(ec.clone());
    ...
    let LoopStartOutcome::Started { version } =
        s1.start_or_resume_loop(&loop_id, "owner1", 300).await.unwrap()
    else { panic!("not started") };
    let v = version.unwrap();
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let ws = AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() };

    let (r1, r2) = tokio::join!(
        s1.prepare_next_attempt(&loop_id, "owner1", v, l.fencing_token, "p1", ws.clone(), None),
        s2.prepare_next_attempt(&loop_id, "owner1", v, l.fencing_token, "p1", ws.clone(), None),
    );
    ...
    assert_eq!(ac.load(Ordering::SeqCst), 1, "loser must have 0 attempt effects");
}
...
async fn test_two_pool_loser_zero_side_effects() {
    ...
    assert_eq!(ac.load(Ordering::SeqCst), 0, "loser attempt effects must be 0");
    assert_eq!(ec.load(Ordering::SeqCst), 0, "loser execution effects must be 0");
    assert_eq!(dc.load(Ordering::SeqCst), 0, "loser decision effects must be 0");
}
...
async fn test_owner_takeover_blocks_old_owner() {
    ...
    let (loop_id, _v, _ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-oto", "hoto").await;
    ...
    assert!(
        ...
        "owner takeover must block different owner: {:?}", r
    );
}
...
async fn test_stale_fencing_rejected() {
    let (db, gw) = setup().await;
    let svc = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw);
    let (loop_id, _v, _ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-sfr", "hsfr").await;

    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, l.fencing_token + 99, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
        None,
    ).await;
    assert!(
        matches!(r, Ok(PrepareAttemptOutcome::OwnershipLost)),
        "stale fencing must be rejected: {:?}", r
    );
}
...
async fn test_gp01_first_attempt_passes() {
    ...
    let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
    let r = svc.prepare_next_attempt(
        &loop_id, "owner1", l.version, l.fencing_token, "prof-1",
        AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
        None,
    ).await.unwrap();
    let PrepareAttemptOutcome::Prepared { attempt_id, ordinal, .. } = r else { panic!("{:?}", r) };
    assert_eq!(ordinal, 1);
    let _ = svc.dispatch_attempt(&attempt_id, "t1", "prof-1", None, None, "ik-gp01d", "hgp01d").await.unwrap();
...
async fn test_gp02_one_repair_then_pass() {
    ...
    let r1 = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw.clone())
        .prepare_next_attempt(&loop_id, "owner1", l.version, l.fencing_token, "prof-1",
            AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() }, None,
        ).await.unwrap();
    let PrepareAttemptOutcome::Prepared { attempt_id: a1_id, .. } = r1 else { panic!("{:?}", r1) };
    let _ = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw.clone())
        .dispatch_attempt(&a1_id, "t1", "prof-1", None, None, "ik-gp02a", "hgp02a").await.unwrap();

    // Decision: ContinueRepair (manual classification).
    let di = DecisionInput {
        outcome_result: Some("failed".into()),
        ownership_fencing_ok: true, worktree_identity_ok: true,
    ...
    let info = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw.clone())
        .inspect_loop(&loop_id).await.unwrap().unwrap();
    assert_eq!(info.attempt_count, 1, "one attempt created (repair needs manual loop advancement)");
}
...
async fn test_gp03_progressive_repairs_budget_allows() {
    ...
    let r = policy.check_can_attempt(3, 0, 0, 0, Some(100), Some(200), Some(300), None, Some(1000), None, true);
    assert!(matches!(r, BudgetCheckResult::Ok), "3 attempts within budget: {:?}", r);
}
...
async fn test_gp04_no_progress_stop() {
    ...
    let r = policy.check_can_attempt(1, 3, 0, 0, Some(100), Some(200), Some(300), None, Some(1000), None, true);
    assert!(matches!(r, BudgetCheckResult::Exhausted { .. }), "no-progress stop: {:?}", r);
}
...
async fn test_gp06_hard_attempt_budget() {
    let policy = BudgetPolicy { max_attempts: Some(5), max_attempts_mode: BudgetMode::Hard, ..Default::default() };
    let r = policy.check_can_attempt(5, 0, 0, 0, Some(100), Some(200), Some(300), None, Some(1000), None, true);
    assert!(matches!(r, BudgetCheckResult::Exhausted { .. }), "hard attempt budget: {:?}", r);
}
...
async fn test_gp07_unknown_token_usage() {
    ...
    let policy = BudgetPolicy { unknown_usage_policy: UnknownUsagePolicy::AllowWithWarning, ..Default::default() };
    ...
    assert!(
        ...
        "AllowWithWarning passes or flags: {:?}", r
    );
    ...
    let policy2 = BudgetPolicy {
        ...
        max_input_tokens: Some(1000), max_input_tokens_mode: BudgetMode::Hard,
        ..Default::default()
    };
    ...
    assert!(
        matches!(r2, BudgetCheckResult::Unknown { .. }) || matches!(r2, BudgetCheckResult::Exhausted { .. }),
        "BlockUnknown must flag unknown usage: {:?}", r2
    );

    // AwaitHuman: passes but flags human intervention needed.
    let policy3 = BudgetPolicy { unknown_usage_policy: UnknownUsagePolicy::AwaitHuman, ..Default::default() };
...
async fn test_gp08_hard_token_budget() {
    let policy = BudgetPolicy {
        max_input_tokens: Some(500), max_input_tokens_mode: BudgetMode::Hard,
        max_output_tokens: Some(500), max_output_tokens_mode: BudgetMode::Hard,
        max_total_tokens: Some(1000), max_total_tokens_mode: BudgetMode::Hard,
        ..Default::default()
    };
    let r = policy.check_can_attempt(1, 0, 0, 0, Some(600), Some(100), Some(700), None, None, None, true);
    assert!(matches!(r, BudgetCheckResult::Exhausted { .. }), "hard token budget: {:?}", r);

    let r2 = policy.check_can_attempt(1, 0, 0, 0, Some(100), Some(600), Some(700), None, None, None, true);
    assert!(matches!(r2, BudgetCheckResult::Exhausted { .. }), "hard output token budget: {:?}", r2);

    let r3 = policy.check_can_attempt(1, 0, 0, 0, Some(500), Some(500), Some(1001), None, None, None, true);
    assert!(matches!(r3, BudgetCheckResult::Exhausted { .. }), "hard total token budget: {:?}", r3);
}

// S09: Hard Tool Call Budget
#[tokio::test]
async fn test_gp09_hard_tool_call_budget() {
    let policy = BudgetPolicy { max_tool_calls: Some(10), max_tool_calls_mode: BudgetMode::Hard, ..Default::default() };
    let r = policy.check_can_attempt(1, 0, 0, 0, None, None, None, Some(15), None, None, true);
    assert!(matches!(r, BudgetCheckResult::Exhausted { .. }), "hard tool call budget: {:?}", r);
}
...
async fn test_gp10_hard_cost_budget() {
    let policy = BudgetPolicy {
        max_estimated_cost_micros: Some(1000), max_estimated_cost_micros_mode: BudgetMode::Hard,
        ..Default::default()
    };
    let r = policy.check_can_attempt(1, 0, 0, 0, None, None, None, None, None, Some(1500), true);
    assert!(matches!(r, BudgetCheckResult::Exhausted { .. }), "hard cost budget: {:?}", r);
}

// S11-S12: Infrastructure Blocked / Reconciliation Required
#[tokio::test]
async fn test_gp11_infrastructure_blocked() {
    let di = DecisionInput { infrastructure_blocked: true, ownership_fencing_ok: true, worktree_identity_ok: true, ..Default::default() };
    assert_eq!(di.classify(), DecisionClassification::InfrastructureBlocked);
}

#[tokio::test]
async fn test_gp12_reconciliation_required() {
    let di = DecisionInput { i4_reconciliation_required: true, ..Default::default() };
    assert_eq!(di.classify(), DecisionClassification::AwaitingReconciliation);
}

// S13: Awaiting Human
#[tokio::test]
async fn test_gp13_awaiting_human() {
    let di = DecisionInput { security_blocker: true, ownership_fencing_ok: true, worktree_identity_ok: true, ..Default::default() };
...
async fn test_gp14_project_escalation() {
    let di = DecisionInput { task_scope_insufficient: true, ownership_fencing_ok: true, worktree_identity_ok: true, ..Default::default() };
    assert_eq!(di.classify(), DecisionClassification::EscalateToProjectPlanner);
}

// S15-S16: Cancellation scenarios
#[tokio::test]
async fn test_gp15_cancellation_classification() {
    let di = DecisionInput { cancellation_requested: true, ..Default::default() };
...
async fn test_gp16_cancellation_overrides() {
    ...
    let di = DecisionInput {
        ...
        all_required_steps_passed: true, evidence_complete: true,
        dossier_present: true, dossier_fingerprint_matches: true,
        ownership_fencing_ok: true, worktree_identity_ok: true,
        ..Default::default()
    };
    assert_eq!(di.classify(), DecisionClassification::Cancelled,
...
async fn test_gp23_two_pool_full_controller() {
    ...
    let ac = Arc::new(AtomicUsize::new(0));
    let s1 = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone()).with_loop_create_count(lc.clone()).with_attempt_create_count(ac.clone());
    let s2 = TaskEngineeringLoopService::new(db.pool.clone())
        .with_i4_gateway(gw.clone()).with_loop_create_count(lc.clone()).with_attempt_create_count(ac.clone());
...
async fn test_gp26_profile_selection_all_scenarios() {
    ...
    assert_eq!(policy.select(&[("prof-c", None), ("prof-b", None), ("prof-a", None)]), Some("prof-b".into()));

    // Switch forbidden
    assert!(!LoopProfilePolicy::default().can_switch(0, Some("a"), Some("b")));

    // Switch allowed within provider
    let p2 = LoopProfilePolicy { allow_profile_switch: true, forbidden_provider_changes: true, ..Default::default() };
    assert!(p2.can_switch(0, Some("anthropic"), Some("anthropic")));
    assert!(!p2.can_switch(0, Some("anthropic"), Some("openai")));

    // Max switches
    let p3 = LoopProfilePolicy { allow_profile_switch: true, max_profile_switches: 2, ..Default::default() };
    assert!(p3.can_switch(0, None, None));
    assert!(p3.can_switch(1, None, None));
    assert!(!p3.can_switch(2, None, None));

    // Allowlist rejection
    assert!(!LoopProfilePolicy { allowed_profile_ids: vec!["prof-a".into()], ..Default::default() }.is_allowed("prof-b"));
}
...
async fn test_gp27_context_security() {
    ...
    let spec = ContextPackSpec {
        task_id: "t1".into(), task_goal: "fix bug".into(),
        acceptance_criteria: "tests pass".into(), attempt_ordinal: 1,
        workspace_continuation: AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() },
        previous_outcome: None, primary_failure_classification: None,
        all_blockers: vec![], failed_required_steps: vec![],
        evidence_refs: vec![], changed_files: vec![],
        do_not_repeat_fingerprints: vec![],
        required_next_objective: None, remaining_budget_facts: None,
        runtime_profile_id: "p1".into(), stop_conditions: vec![],
    };
...
async fn test_cli_dry_run_zero_writes() {
    let (db, gw) = setup().await;
    let svc = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw);
    let (loop_id, _v, _ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-cli", "hcli").await;

    // Take a DB fingerprint before dry-run.
    let before: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM task_engineering_loops").fetch_one(&db.pool).await.unwrap();
    let before_attempts: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM task_engineering_attempts").fetch_one(&db.pool).await.unwrap();
    ...
    let after: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM task_engineering_loops").fetch_one(&db.pool).await.unwrap();
    let after_attempts: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM task_engineering_attempts").fetch_one(&db.pool).await.unwrap();
    assert_eq!(before.0, after.0, "inspect must be read-only: loops");
    assert_eq!(before_attempts.0, after_attempts.0, "inspect must be read-only: attempts");
}
...
async fn test_cli_inspect_shows_durable_facts() {
    let (db, _gw) = setup().await;
    let svc = TaskEngineeringLoopService::new(db.pool.clone());
    let (loop_id, _v, _ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-insp", "hinsp").await;
    let info = svc.inspect_loop(&loop_id).await.unwrap().unwrap();
...
async fn test_cli_cancel_formal() {
    let (db, gw) = setup().await;
    let svc = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw);
    let (loop_id, v, ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-cancel", "hcancel").await;
...
async fn test_cli_resume_idempotent() {
    let (db, gw) = setup().await;
    let svc = TaskEngineeringLoopService::new(db.pool.clone()).with_i4_gateway(gw);
    let (loop_id, _v, _ft) = create_and_start(&db, Arc::new(FixtureI4Gateway::new(db.pool.clone())), "ik-resume", "hresume").await;
    let r = svc.start_or_resume_loop(&loop_id, "owner1", 300).await.unwrap();
    assert!(
        matches!(r, LoopStartOutcome::AlreadyOwned { .. }) || matches!(r, LoopStartOutcome::Resumed { .. }),
        "resume must be idempotent: {:?}", r
    );
}
...
async fn test_repeat_two_pool_attempt_creation_100() {
    for run in 1..=100 {
        ...
        let ac = Arc::new(AtomicUsize::new(0));
        let s1 = TaskEngineeringLoopService::new(db.pool.clone())
            .with_i4_gateway(gw.clone()).with_attempt_create_count(ac.clone());
        let s2 = TaskEngineeringLoopService::new(db.pool.clone())
            .with_i4_gateway(gw.clone()).with_attempt_create_count(ac.clone());

        let CreateLoopOutcome::Created { loop_id } =
            s1.create_loop(&loop_req("ik-rpa", "hrpa")).await.unwrap()
        else { panic!("not created at run {run}") };
        let LoopStartOutcome::Started { version } =
            s1.start_or_resume_loop(&loop_id, "owner1", 300).await.unwrap()
        else { panic!("not started at run {run}") };
        let v = version.unwrap();
        let l = TaskLoopRepo::new(db.pool.clone()).load_loop(&loop_id).await.unwrap().unwrap();
        let ws = AttemptWorkspaceSource::InitialTaskWorkspace { repository_path: "/tmp/r".into() };
        ...
        assert_eq!(ac.load(Ordering::SeqCst), 1, "run {run}: loser must have 0 attempt effects");
