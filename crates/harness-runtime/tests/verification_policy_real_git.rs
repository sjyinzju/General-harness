...
async fn setup_real_service(
    ...
    let known_secrets: Vec<String> = files_for_secret_scan
        ...
        .collect();
    let secret_scanner = harness_runtime::policy::scanner::SecretScanner::new(known_secrets);
...
async fn test_real_git_clean_worktree() {
    ...
    let (svc, _db, _db_td) = setup_real_service(git, vec![]).await;
    ...
    assert_eq!(
        svc.scan_start_count.load(Ordering::SeqCst),
        1,
        "validator must start"
    );
}
...
async fn test_real_git_secret_in_untracked_file() {
    ...
    std::fs::write(repo.path().join(".env"), format!("API_KEY={secret}\n")).unwrap();
    let git = GitRunner::new(repo.path().join("scratch9")).unwrap();

    let content = std::fs::read(repo.path().join(".env")).unwrap();
    let (svc, db, _db_td) = setup_real_service(git, vec![(".env".into(), content)]).await;
...
async fn test_real_git_baseline_mismatch_detected() {
    ...
    assert!(
        matches!(r, PolicyStepOutcome::Completed { .. })
            || matches!(r, PolicyStepOutcome::InfrastructureError { .. }),
...
async fn test_real_git_two_pool_one_scanner() {
    ...
    let (r1, r2) = tokio::join!(sv1.execute_policy_step(&rq), sv2.execute_policy_step(&rq));
