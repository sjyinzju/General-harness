...
async fn setup_db() -> Database {
    ...
    sqlx::query("INSERT INTO tasks(id,project_id,goal,lifecycle) VALUES('t1','p1','test','verified')")
        .execute(&db.pool).await.unwrap();
...
async fn seed_candidate(pool: &SqlitePool, candidate_id: &str, executor_profile_id: &str,
    base_commit: &str, tree_hash: &str, diff_digest: &str) {
    sqlx::query(
...
async fn seed_review(pool: &SqlitePool, review_id: &str, candidate_id: &str, reviewer_profile_id: &str, state: &str) {
    sqlx::query(
...
fn setup_git_repo() -> (TempDir, String) {
    ...
    Command::new("git").args(["config", "user.name", "Test Author"]).current_dir(repo_path).output().unwrap();
    Command::new("git").args(["config", "user.email", "author@test.com"]).current_dir(repo_path).output().unwrap();
    std::fs::write(repo_path.join("README.md"), "# Test\n").unwrap();
    Command::new("git").args(["add", "README.md"]).current_dir(repo_path).output().unwrap();
...
fn get_tree_oid(repo_path: &std::path::Path) -> String {
    String::from_utf8_lossy(
        &Command::new("git").args(["rev-parse", "HEAD^{tree}"]).current_dir(repo_path).output().unwrap().stdout,
    ).trim().to_string()
}
...
async fn test_admission_non_approved_review_blocked() {
    ...
    seed_candidate(&db.pool, &candidate_id, "executor1", "base123", "tree123", "diff123").await;
    ...
    let approved = ApprovedCandidate {
        candidate_id: candidate_id.clone(), review_id: review_id.into(),
        candidate_tree_hash: "tree123".into(), diff_digest: "diff123".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let admission = svc.validate_admission(&approved).await.unwrap();
    assert!(matches!(admission, CommitAdmission::Blocked { .. }), "Expected Blocked, got: {admission:?}");
}
...
async fn test_admission_stale_candidate() {
    ...
    let approved = ApprovedCandidate {
        candidate_id: candidate_id.clone(), review_id: review_id.into(),
        candidate_tree_hash: "wrong_tree".into(), diff_digest: "diff123".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let admission = svc.validate_admission(&approved).await.unwrap();
    assert!(matches!(admission, CommitAdmission::Stale { .. }), "Expected Stale, got: {admission:?}");
}
...
async fn test_admission_diff_digest_mismatch_stale() {
    ...
    let approved = ApprovedCandidate {
        candidate_id: candidate_id.clone(), review_id: review_id.into(),
        candidate_tree_hash: "tree123".into(), diff_digest: "wrong_diff".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let admission = svc.validate_admission(&approved).await.unwrap();
    assert!(matches!(admission, CommitAdmission::Stale { .. }), "Expected Stale, got: {admission:?}");
...
async fn test_admission_reviewer_equals_executor_blocked() {
    let db = setup_db().await;
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "same_profile", "base123", "tree123", "diff123").await;
    let review_id = "rev-same";
    seed_review(&db.pool, review_id, &candidate_id, "same_profile", "approved").await;
...
async fn test_create_commit_stable_oid() {
    ...
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "executor1", &base_oid, &tree_oid, "diff456").await;
    ...
    let approved = ApprovedCandidate {
        candidate_id: candidate_id.clone(), review_id: review_id.into(),
        candidate_tree_hash: tree_oid.clone(), diff_digest: "diff456".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let author = GitIdentity::new("Test Author", "author@test.com");
    let committer = GitIdentity::new("Test Committer", "committer@test.com");

    let outcome = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &committer, "feat: test", &repo_path).await.unwrap();
    ...
    let verify = Command::new("git").args(["cat-file", "-e", &outcome.commit_candidate.commit_oid]).current_dir(&repo_path).status().unwrap();
    assert!(verify.success());

    // Verify trailers in commit message
    let msg = String::from_utf8_lossy(
        &Command::new("git").args(["log", "-1", "--format=%B", &outcome.commit_candidate.commit_oid]).current_dir(&repo_path).output().unwrap().stdout,
    ).to_string();
    assert!(msg.contains("Harness-Candidate:"), "missing Harness-Candidate trailer: {msg}");
}
...
async fn test_create_commit_idempotent_same_oid() {
    ...
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "executor1", &base_oid, &tree_oid, "diff789").await;
    ...
    let approved = ApprovedCandidate {
        candidate_id: candidate_id.clone(), review_id: review_id.into(),
        candidate_tree_hash: tree_oid.clone(), diff_digest: "diff789".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let author = GitIdentity::new("A", "a@test.com");
    let committer = GitIdentity::new("C", "c@test.com");

    let o1 = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &committer, "msg", &repo_path).await.unwrap();
    let o2 = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &committer, "msg", &repo_path).await.unwrap();
    assert_eq!(o1.commit_candidate.commit_oid, o2.commit_candidate.commit_oid);
    assert!(o2.recovered);
}
...
async fn test_create_commit_does_not_modify_user_index() {
    ...
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "executor1", &base_oid, &tree_oid, "diff_idx").await;
    let review_id = "rev-idx";
    seed_review(&db.pool, review_id, &candidate_id, "reviewer1", "approved").await;

    let idx_before = String::from_utf8_lossy(
        &Command::new("git").args(["ls-files", "--stage"]).current_dir(&repo_path).output().unwrap().stdout,
    ).to_string();

    let svc = ControlledCommitService::new(db.pool.clone());
    let approved = ApprovedCandidate {
        candidate_id, review_id: review_id.into(),
        candidate_tree_hash: tree_oid, diff_digest: "diff_idx".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let author = GitIdentity::new("A", "a@t.com");
    svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &author, "msg", &repo_path).await.unwrap();
...
async fn test_create_commit_does_not_modify_worktree() {
    ...
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "executor1", &base_oid, &tree_oid, "diff_wt").await;
    ...
    let approved = ApprovedCandidate {
        candidate_id, review_id: review_id.into(),
        candidate_tree_hash: tree_oid, diff_digest: "diff_wt".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let author = GitIdentity::new("A", "a@t.com");
    svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &author, "msg", &repo_path).await.unwrap();
    ...
    assert!(String::from_utf8_lossy(&status.stdout).trim().is_empty(), "worktree dirty after commit");
}
...
async fn test_recovery_git_object_before_db_write() {
    ...
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "executor1", &base_oid, &tree_oid, "diff_rec").await;
    ...
    let approved = ApprovedCandidate {
        candidate_id: candidate_id.clone(), review_id: review_id.into(),
        candidate_tree_hash: tree_oid.clone(), diff_digest: "diff_rec".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let author = GitIdentity::new("A", "a@t.com");

    let o1 = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &author, "msg", &repo_path).await.unwrap();
    assert!(!o1.recovered);

    // Simulate crash: delete DB record but keep Git object
    sqlx::query("DELETE FROM commit_candidates WHERE commit_request_id = ?")
        .bind(&o1.commit_candidate.commit_request_id).execute(&db.pool).await.unwrap();
    sqlx::query("UPDATE commit_requests SET state = 'materializing' WHERE commit_request_id = ?")
        .bind(&o1.commit_candidate.commit_request_id).execute(&db.pool).await.unwrap();

    let o2 = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &author, "msg", &repo_path).await.unwrap();
...
async fn test_create_commit_unique_candidate_per_scope() {
    ...
    let candidate_id = format!("cand-{}", Uuid::new_v4());
    seed_candidate(&db.pool, &candidate_id, "executor1", &base_oid, &tree_oid, "diff_uniq").await;
    ...
    let approved = ApprovedCandidate {
        candidate_id, review_id: review_id.into(),
        candidate_tree_hash: tree_oid, diff_digest: "diff_uniq".into(),
        review_decision_digest: format!("digest-{review_id}"), approved_at: Utc::now(),
    };
    let author = GitIdentity::new("A", "a@t.com");

    let o1 = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &author, "msg A", &repo_path).await.unwrap();
    let o2 = svc.create_commit(&approved, "repo-test", "refs/heads/main", &author, &author, "msg A", &repo_path).await.unwrap();
    // Same scope → same CommitCandidate
    assert_eq!(o1.commit_candidate.commit_oid, o2.commit_candidate.commit_oid);
    assert_eq!(o1.commit_candidate.commit_request_id, o2.commit_candidate.commit_request_id);
}
