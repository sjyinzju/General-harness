...
impl ApprovalValidator {
    ...
    pub async fn validate_and_consume(
        ...
        verification_run_id: &str, step_id: &str, step_op_id: &str,
        cmd_fingerprint: &str, worktree_id: &str, fencing: i64,
    ) -> ApprovalDecision {
        // Load approval.
        let row: Option<(String, String, String, String, String, String, i64, bool, String, Option<String>)> = sqlx::query_as(
        ...
        let (aid, vr, si, so, cf, wt, fence, single_use, lc, expires) = match row {
            Some(r) => r,
            None => return ApprovalDecision::Denied { reason: "approval not found".into() },
        };

        // Check lifecycle.
        match lc.as_str() {
            "consumed" => return ApprovalDecision::AlreadyConsumed { approval_id: aid },
            "expired" | "revoked" => return ApprovalDecision::Denied { reason: format!("approval {lc}") },
            "pending" => {},
            _ => return ApprovalDecision::Denied { reason: format!("unknown lifecycle: {lc}") },
        }
        ...
        if let Some(ref exp) = expires {
            if let Ok(exp_dt) = chrono::NaiveDateTime::parse_from_str(exp, "%Y-%m-%d %H:%M:%S") {
                ...
                if chrono::Utc::now() > exp_utc {
                    let _ = sqlx::query("UPDATE verification_approvals SET lifecycle='expired' WHERE approval_id=?")
                        .bind(&aid).execute(&self.pool).await;
                    return ApprovalDecision::Denied { reason: "approval expired".into() };
                }
            }
        }

        // Validate identity bindings.
        if vr != verification_run_id { return ApprovalDecision::Denied { reason: "wrong run".into() }; }
        if si != step_id { return ApprovalDecision::Denied { reason: "wrong step".into() }; }
        if so != step_op_id { return ApprovalDecision::Denied { reason: "wrong operation".into() }; }
        if cf != cmd_fingerprint { return ApprovalDecision::Denied { reason: "wrong fingerprint".into() }; }
        if wt != worktree_id { return ApprovalDecision::Denied { reason: "wrong worktree".into() }; }
        if fence != fencing { return ApprovalDecision::Denied { reason: "stale fencing".into() }; }
        ...
        if single_use {
            ...
            match rows {
                ...
                Err(e) => return ApprovalDecision::Error { reason: format!("consume: {e}") },
            }
        }
    ...
    pub async fn insert_approval(
        &self, approval_id: &str, verification_run_id: &str, step_id: &str,
        step_op_id: &str, cmd_fingerprint: &str, worktree_id: &str,
        fencing: i64, single_use: bool, expires_at: Option<&str>,
    ) -> Result<(), CoreError> {
        sqlx::query("INSERT OR REPLACE INTO verification_approvals (approval_id, verification_run_id, step_id, step_op_id, cmd_fingerprint, worktree_id, fencing_token, single_use, lifecycle, expires_at) VALUES (?,?,?,?,?,?,?,?,'pending',?)")
            .bind(approval_id).bind(verification_run_id).bind(step_id).bind(step_op_id)
            .bind(cmd_fingerprint).bind(worktree_id).bind(fencing)
    ...
    pub async fn revoke(&self, approval_id: &str) -> Result<(), CoreError> {
        sqlx::query("UPDATE verification_approvals SET lifecycle='revoked' WHERE approval_id=?")
            .bind(approval_id).execute(&self.pool).await
            .map_err(|e| CoreError::new(ErrorCode::PersistenceError, format!("revoke: {e}"), ErrorSource::System))?;
        Ok(())
    }
}
...
mod tests {
    ...
    async fn setup() -> (ApprovalValidator, Database) {
        ...
        sqlx::query("INSERT INTO projects(id,objective,lifecycle) VALUES('p1','t','active')").execute(&db.pool).await.unwrap();
        sqlx::query("INSERT INTO tasks(id,project_id,goal,lifecycle) VALUES('t1','p1','t','submitted')").execute(&db.pool).await.unwrap();
    ...
    #[tokio::test] async fn test_valid_approval() {
        let (v, _db) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,None).await.unwrap();
        let r = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",5).await;
        assert_eq!(r, ApprovalDecision::Approved { approval_id: "a1".into() });
    }
    #[tokio::test] async fn test_missing_approval() {
        let (v, _) = setup().await;
        let r = v.validate_and_consume("no-exist","r1","s1","o1","cf","w1",5).await;
        assert!(matches!(r, ApprovalDecision::Denied { .. }));
    }
    #[tokio::test] async fn test_wrong_run() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,None).await.unwrap();
        let r = v.validate_and_consume("a1","wrong-run","s1","op1","cf1","wt1",5).await;
        assert!(matches!(r, ApprovalDecision::Denied { .. }));
    }
    #[tokio::test] async fn test_wrong_worktree() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,None).await.unwrap();
        let r = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt99",5).await;
        assert!(matches!(r, ApprovalDecision::Denied { .. }));
    }
    #[tokio::test] async fn test_stale_fencing() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,None).await.unwrap();
        let r = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",99).await;
        assert!(matches!(r, ApprovalDecision::Denied { .. }));
    }
    #[tokio::test] async fn test_single_use_consumed_once() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,true,None).await.unwrap();
        let r1 = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",5).await;
        assert_eq!(r1, ApprovalDecision::Approved { approval_id: "a1".into() });
        let r2 = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",5).await;
        assert!(matches!(r2, ApprovalDecision::AlreadyConsumed { .. }));
    }
    #[tokio::test] async fn test_single_use_response_lost_idempotent() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,true,None).await.unwrap();
        // First consume succeeds.
        v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",5).await;
        // Same operation retry — already consumed, but identity matches.
        let r = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",5).await;
        assert!(matches!(r, ApprovalDecision::AlreadyConsumed { .. }));
    }
    #[tokio::test] async fn test_expired_approval() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,Some("2020-01-01 00:00:00")).await.unwrap();
        let r = v.validate_and_consume("a1","run-1","s1","op1","cf1","wt1",5).await;
        assert!(matches!(r, ApprovalDecision::Denied { .. }));
    }
    #[tokio::test] async fn test_revoked_approval() {
        let (v, _) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,None).await.unwrap();
    ...
    #[tokio::test] async fn test_approval_no_credential_in_db() {
        let (v, db) = setup().await;
        v.insert_approval("a1","run-1","s1","op1","cf1","wt1",5,false,None).await.unwrap();
        let row: (String, String,) = sqlx::query_as("SELECT approval_id, lifecycle FROM verification_approvals WHERE approval_id='a1'")
