...
impl OperationManager {
    ...
    pub async fn try_claim_operation(&self, operation_id: &str, lease_secs: u32) -> Result<Option<String>, CoreError> {
    ...
    pub async fn renew_operation_claim(&self, operation_id: &str, token: &str, lease_secs: u32) -> Result<(), CoreError> {
        let expires = expires_sql(lease_secs);
        ...
        if r.rows_affected() == 0 { return Err(CoreError::new(ErrorCode::PersistenceError, "operation_claim_conflict", ErrorSource::System)); }
        Ok(())
    }

    pub async fn release_operation_claim(&self, operation_id: &str, token: &str) -> Result<(), CoreError> {
    ...
    pub async fn complete_claimed_operation(&self, operation_id: &str, token: &str, result: &serde_json::Value) -> Result<(), CoreError> {
        ...
        if r.rows_affected() == 0 { return Err(CoreError::new(ErrorCode::PersistenceError, "operation_claim_conflict: token mismatch or already terminal", ErrorSource::System)); }
        Ok(())
    }

    pub async fn fail_claimed_operation(&self, operation_id: &str, token: &str, reason: &str) -> Result<(), CoreError> {
    ...
    pub async fn complete(&self, operation_id: &str, result: &serde_json::Value) -> Result<(), CoreError> {
        ...
        if r.rows_affected() == 0 { return Err(CoreError::new(ErrorCode::PersistenceError, "operation not found or already terminal", ErrorSource::System)); }
        Ok(())
    }
