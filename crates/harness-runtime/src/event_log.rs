...
pub async fn get_by_stream(pool: &SqlitePool, stream_id: &str, since_version: Option<u32>) -> Result<Vec<EventLogEntry>, CoreError> {
    ...
    Ok(rows.into_iter().map(|r| EventLogEntry {
        id: r.id, stream_id: r.stream_id, stream_version: r.stream_version as u32,
        event_type: r.event_type, payload_json: r.payload_json, schema_version: r.schema_version as u32,
        correlation_id: r.correlation_id, causation_id: r.causation_id,
        idempotency_key: r.idempotency_key, source: r.source, timestamp: r.created_at,
    }).collect())
}
