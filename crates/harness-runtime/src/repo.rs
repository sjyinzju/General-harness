...
use harness_core::contracts::repository::{
    EventLogEntry, EventLogRepository, ExecutionRecord, ExecutionRepository, OperationRecord,
    OperationRepository, ProjectRepository, TaskRepository, WorkspaceLeaseRecord,
...
impl ProjectRepository for SqlProjectRepo {
    ...
    async fn get(&self, id: &str) -> Result<Option<Project>, CoreError> {
        ...
        Ok(row.map(|r| Project {
            ...
            lifecycle: parse_pl(&r.lifecycle),
            goal_contract_version: r.goal_contract_version.map(|v| v as u32),
            plan_version: r.plan_version.map(|v| v as u32),
        }))
    }
...
struct ProjectRow {
    ...
    objective: String,
    lifecycle: String,
    goal_contract_version: Option<i64>,
...
pub struct SqlExecutionRepo {
    pool: SqlitePool,
}
impl SqlExecutionRepo {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}
...
impl WorkspaceLeaseRepository for SqlLeaseRepo {
    async fn acquire(
        &self,
        l: &harness_core::contracts::workspace::WorkspaceLease,
    ) -> Result<(), CoreError> {
    ...
    async fn get(&self, id: &str) -> Result<Option<WorkspaceLeaseRecord>, CoreError> {
        let row = sqlx::query_as::<_, LeaseRow>("SELECT id, task_id, lifecycle, worktree_path, branch_name FROM workspace_leases WHERE id = ?")
            .bind(id).fetch_optional(&self.pool).await.map_err(map_err)?;
        Ok(row.map(|r| WorkspaceLeaseRecord {
            id: r.id,
            task_id: r.task_id,
            lifecycle: r.lifecycle,
...
pub struct SqlEventLogRepo {
    pool: SqlitePool,
}
impl SqlEventLogRepo {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}
...
pub struct SqlOperationRepo {
    pool: SqlitePool,
}
impl SqlOperationRepo {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}
