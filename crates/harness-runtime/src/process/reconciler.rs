...
impl ProcessReconciler {
    pub fn new(pool: SqlitePool, registry: Arc<ProcessRegistry>, supervisor_id: String) -> Self {
        Self { pool, registry, supervisor_instance_id: supervisor_id }
    }
    ...
    pub async fn reconcile(&self) -> Result<Vec<String>, CoreError> {
        ...
        ).fetch_all(&self.pool).await.map_err(|e| CoreError::new(ErrorCode::PersistenceError, e.to_string(), ErrorSource::System))?;
        ...
        for (exec_id, _lc) in &rows {
            if !self.registry.is_alive(exec_id).await {
                let idempotency_key = format!("reconcile-lost-{exec_id}-{}", self.supervisor_instance_id);
                match svc.transition_execution(
                    exec_id,
