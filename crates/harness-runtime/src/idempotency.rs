...
pub async fn renew_claim(
    ...
    if r.rows_affected() == 0 {
        return Err(CoreError::new(
            ErrorCode::PersistenceError,
            "claim not found or token mismatch",
            ErrorSource::System,
        ));
    }
    Ok(())
...
pub async fn complete_claim(
    ...
    if r.rows_affected() == 0 {
        return Err(CoreError::new(
            ErrorCode::PersistenceError,
            "claim not found, token mismatch, or not pending",
            ErrorSource::System,
        ));
    }
    Ok(())
...
pub async fn is_duplicate(pool: &SqlitePool, key: &str) -> Result<bool, CoreError> {
    let row: Option<(String,)> = sqlx::query_as(
        "SELECT key FROM idempotency_records WHERE key = ? AND status = 'completed'",
    )
    .bind(key)
    .fetch_optional(pool)
    .await
...
mod tests {
    ...
    async fn test_claim_complete_read() {
        ...
        complete_claim(&db.pool, key, &token, r#""ok""#)
            .await
            .unwrap();
    ...
    async fn test_old_owner_cannot_complete_after_takeover() {
        ...
        let token1 = try_claim(&db.pool, key, "hash-1", 1)
            .await
            .unwrap()
            .unwrap();
        // Let lease expire (lease=1s, sleep=3s for safety margin)
        tokio::time::sleep(std::time::Duration::from_secs(3)).await;
        let token2 = try_claim(&db.pool, key, "hash-1", 60)
            .await
            .unwrap()
            .unwrap();
        ...
        complete_claim(&db.pool, key, &token2, r#""fresh""#)
            .await
            .unwrap();
    }
}
